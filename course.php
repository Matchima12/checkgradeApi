...
header ("Access-Contro1-Allow-Methods: GET, POST, PATCH, PUT, DELETE, OPTIONS"); 
...
if(isset($_GET['fn'])){
    ...
    $courseQuery = $conn ->prepare($query);
    $courseQuery -> execute(array(':CourseID' => $_GET['CourseID']));
    $courseFetch = array(
        'Status' =>true,
    ...
    $courseQuery = $conn ->prepare($query);
    $courseQuery -> execute(array(':CourseName' => '%'.$_GET['CourseName'].'%'));
    $courseFetch = array(
        'Status' =>true,
        'Data' =>$courseQuery ->fetch(PDO::FETCH_ASSOC)
    );
